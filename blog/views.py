...
def blog_list(request,username):
    ...
    context['introduction']=UserInfo.objects.get(user=user).introduction
    context['blogs_number']=blogs_all_list.count()
    context['stars_number']=Star.objects.filter(user=user).count()
...
def blog_detail(request,blog_pk):
    ...
    try:# 有可能用户没有登录
        ...
        pass
    star_number=blog.star_set.all().count()# 这篇博客的收藏人数
    context['is_star']=is_star
    context['star_number']=star_number
    #阅读量加一
    ...
    context['introduction']=UserInfo.objects.get(user=blog.author).introduction
    context['blogs_number']=Blog.objects.filter(author=blog.author).count()
    context['stars_number']=Star.objects.filter(user=blog.author).count()
    blog_types=BlogType.objects.filter(author=blog.author)
    my_blog_types=[]
    for blog_type in blog_types:# 添加每种类型对应的博客数量，利用python可以临时添加实例变量和类变量的属性
        blog_type.blog_count = Blog.objects.filter(blog_type=blog_type).count()
        my_blog_types.append(blog_type)
    ...
    response.set_cookie('blog_%s_read'%blog_pk,'true',max_age=900)
    return response
...
def blogs_with_type(request,blog_type_pk):#参数变量名必须与urls中的相同
    ...
    context['blog_types']=my_blog_types
    context['author']=blog_type.author
    context['introduction']=UserInfo.objects.get(user=blog_type.author).introduction
    context['blogs_number']=Blog.objects.filter(author=blog_type.author).count()
    context['stars_number']=Star.objects.filter(user=blog_type.author).count()
